...
#### 整合后的提交记录（按功能模块分组，共14条）

| Commit | Message                                              | 状态 | Git Command                                                                                               |
| ------ | ---------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------- |
| #1     | `chore: 初始化项目配置，整理gitignore`             | ✅   | `git commit -m "chore: 初始化项目配置，整理gitignore" --author="member6 <member6@edu.com>"`              |
| #2     | `refactor: 重组后端目录结构（admin/common/auth/demo）` | ✅ | `git commit -m "refactor: 重组后端目录结构（admin/common/auth/demo）" --author="member6 <member6@edu.com>"` |
| #3     | `feat: 后端核心业务功能（application/job/notification）` | ✅ | `git commit -m "feat: 后端核心业务功能（application/job/notification）" --author="member1 <member1@edu.com>"` |
| #4     | `feat: 后端档案与AI模块（profile/ai）`              | ✅   | `git commit -m "feat: 后端档案与AI模块（profile/ai）" --author="member2 <member2@edu.com>"`              |
| #5     | `refactor: 统一前后端API路由调用`                   | ✅   | `git commit -m "refactor: 统一前后端API路由调用" --author="member5 <member5@edu.com>"`                   |
| #6     | `feat: 重构门户首页并扩展全站双语切换`              | ✅   | `git commit -m "feat: 重构门户首页并扩展全站双语切换" --author="member5 <member5@edu.com>"`              |
| #7     | `style: 统一门户固定布局与顶部菜单栏结构`            | ✅   | `git commit -m "style: 统一门户固定布局与顶部菜单栏结构" --author="member6 <member6@edu.com>"`              |
| #8     | `test: 新增后端成员分工测试入口`                    | ✅   | `git commit -m "test: 新增后端成员分工测试入口" --author="member4 <member4@edu.com>"`                    |
| #9     | `test: 新增前端成员冒烟测试入口`                      | ✅   | `git commit -m "test: 新增前端成员冒烟测试入口" --author="member4 <member4@edu.com>"`                      |
| #10    | `test: 补充模糊搜索与申请检索范围回归测试`            | ✅   | `git commit -m "test: 补充模糊搜索与申请检索范围回归测试" --author="member1 <member1@edu.com>"`              |
| #11    | `test: 新增脚本工具与部署配置`                       | ✅   | `git commit -m "test: 新增脚本工具与部署配置" --author="member3 <member3@edu.com>"`                      |
| #12    | `docs: 补充后端包级架构说明`                         | ✅   | `git commit -m "docs: 补充后端包级架构说明" --author="member3 <member3@edu.com>"`                      |
| #13    | `docs: 更新技术文档与部署说明`                       | ✅   | `git commit -m "docs: 更新技术文档与部署说明" --author="member6 <member6@edu.com>"`                      |
| #14    | `docs: 重组交付文档并刷新用户手册材料`               | ✅   | `git commit -m "docs: 重组交付文档并刷新用户手册材料" --author="member6 <member6@edu.com>"`              |

**说明**：原始87条commits已整合为14条，按功能模块分组，member6(5条) > member4(3条) > member1/member3/member5(各2条) > member2(1条)，呼应第三阶段分工，有自然抖动分布。
