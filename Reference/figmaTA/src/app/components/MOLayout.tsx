...
import { LayoutDashboard, Users, Settings, ArrowLeftRight, LogOut, User } from "lucide-react";

export function MOLayout() {
  const location = useLocation();

  const navItems = [
    { name: "Overview", path: "/mo/overview", icon: LayoutDashboard },
    { name: "Applicants", path: "/mo", icon: Users },
    { name: "Settings", path: "/mo/settings", icon: Settings },
  ];

  const isActive = (path: string) => {
    if (path === "/mo") {
      return location.pathname === "/mo" || location.pathname.startsWith("/mo/post") || location.pathname.startsWith("/mo/applicant");
    }
    ...
    <div className="flex h-screen w-full bg-[#F5F5F7] font-sans p-3 gap-3">
      ...
      <div className="w-56 flex flex-col flex-shrink-0">
        ...
        <div className="px-4 pt-5 pb-4">
          <span className="text-[17px] text-[#1D1D1F] tracking-tight" style={{ fontWeight: 600 }}>MO Portal</span>
        </div>
        ...
        <div className="px-2 pb-4 space-y-0.5">
          ...
          >
            ...
            <span className="text-[14px]">Switch to TA</span>
          </Link>
          <Link
            to="/login"
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white rounded-2xl shadow-[0_1px_8px_rgba(0,0,0,0.04)] border border-[#E5E5EA]/60 overflow-hidden">
        ...
        <header className="h-14 flex items-center justify-between px-7 bg-white/80 backdrop-blur-xl border-b border-[#F0F0F5] flex-shrink-0 sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#FDE68A] to-[#F59E0B] flex items-center justify-center">
              <User className="w-3.5 h-3.5 text-white" strokeWidth={2} />
            </div>
            <span className="text-[13px] text-[#1D1D1F]" style={{ fontWeight: 500 }}>Michael Scott, MO</span>
          </div>
          <Link
            to="/login"
